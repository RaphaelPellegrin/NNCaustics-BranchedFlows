...
def N_heads_run_Gaussiann(initial_x=0, final_t=50, means=[[7.51, 4.6], [8.78, 6.16]], alpha_=1, width_=40, width_heads=8, \
  ...
  for ne in range(num_epochs):
      ...
      for l in range(number_of_heads):
        ...
        L3=((torch.exp(-t/100)*(px_dot+partial_x))**2).mean()
        L4=((torch.exp(-t/100)*(py_dot+partial_y))**2).mean()
...
if __name__=='__main__':
 ...
 [45.65, 18.15],
 [27.99, 40.4]]
  N_heads_run_Gaussiann(grid_size=4000, means=means_cell,final_t=20, width_=64, width_heads=32, epochs_=50000)
