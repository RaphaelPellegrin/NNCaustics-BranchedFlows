...
def N_heads_run_Gaussiann(initial_x=0, final_t=50, means=[[7.51, 4.6], [8.78, 6.16]], alpha_=1, width_=40, width_heads=8, \
  ...
  for ne in range(num_epochs):
      ...
      for l in range(number_of_heads):
        ...
        # CHANGED
        # Outputs
        x=initial_x+(1-torch.exp(-t))*head[:,0]
        y=initial_y+(1-torch.exp(-t))*head[:,1]
        px=1+(1-torch.exp(-t))*head[:,2]
        py=0+(1-torch.exp(-t))*head[:,3]
        x=x.reshape((-1,1))
        y=y.reshape((-1,1))
        px=px.reshape((-1,1))
        py=py.reshape((-1,1))
...
if __name__=='__main__':
 ...
 [45.65, 18.15],
 [27.99, 40.4]]
  N_heads_run_Gaussiann(grid_size=2000, means=means_cell,final_t=10, width_=64, width_heads=32, epochs_=1000)
