...
def N_heads_run_Gaussiann(initial_x=0, final_t=50, means=[[7.51, 4.6], [8.78, 6.16]], alpha_=1, width_=40, width_heads=8, \
  ...
  for j in range(number_of_heads):
    # Initial conditions
    initial_condition=random.randint(0,5000)/(100*50)
  ...
  for ne in range(num_epochs):
      ...
      for l in range(number_of_heads):
        ...
        # Loss
        L1=((torch.exp(-t)*(x_dot-px))**2).mean()
        L2=((torch.exp(-t)*(y_dot-py))**2).mean()
        ...
        L3=((torch.exp(-t)*(px_dot+partial_x))**2).mean()
        L4=((torch.exp(-t)*(py_dot+partial_y))**2).mean()
...
if __name__=='__main__':
 ...
 [45.65, 18.15],
 [27.99, 40.4]]
  N_heads_run_Gaussiann(grid_size=2000, means=means_cell,final_t=14, width_=64, width_heads=32, epochs_=50000)
